...
if (!function_exists('objtoarr')) {
    ...
    function objtoarr($collection)
    {
       ...
       foreach ($collection as $k =>$v){
           $collection[$k]=$v->toArray();
       }
     return $collection;
    }
}
if (!function_exists('UserArr')) {
    /**
     * 53,推广账号一个或多个问题
     * @param mixed $Data 要转换的数组
     * @param array $resultSet 数组
     * @return \think\model\Collection|\think\Collection
     */
    function UserArr($Data)
    {
      ...
      //  return $result;
        ...
        $result=array_values(array_unique($result));//去除重复数组并排序
        return $result;
    }
}
if (!function_exists('ProjectId_ProIdArr')) {
    /**
     * ProjectId转换为推广账号数组然后UserArr
     * @param mixed $Data 单条字段Id
     * @param mixed $table 查询的表
     * @param mixed $field 查询的字段
     * * @param mixed $Inset 合并的数组
     * @param array $resultSet 数组
     * @return \think\model\Collection|\think\Collection
     */
    function ProjectId_ProIdArr($Data,$table,$field,$InSet=[])
    {
            //接受项目ID传过来的值
            $Id_Arr=Db::table($table)
            ->where('Id',$Data)
            ->field($field)
            ->select();
            $Id_Arr=UserArr($Id_Arr);
            //合并权限数组，并去重，重新排序
            $InSet=array_values(array_unique(array_merge($Id_Arr,$InSet)));
            return $InSet;
    }
}
