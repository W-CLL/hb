...
use app\promotion\model\PromotionCon as ModelPromotionCon;
use think\Cache;
class PromotionCon extends Common
{
    ...
    public function get_ins_promotion_con()
    {
        ...
        $pro_user = Db::table('promotion_user')
            ...
            ->field('Id Pro_Id,Pro_User,Remarks,Rebate,Pro_Psw,Client_Rebate')
            ->select();
        ...
        //查询是否录入,将已录入的数据展示
        $pro = Db::table('promotion_con')->where($date)->where('Project_Id', '=', $Id)->select();
        if ($pro) {
            foreach ($pro as $k => $v) {
                foreach ($data as $k2 => $v2) {
                    if ($v['Pro_Id'] == $v2['Pro_Id']) {
                        $v['Con_B'] = round($v['Money_Con'] * $v2['Rebate'], 2);
                        $v['ConRemarks'] = $v['Remarks'];
                        $data[$k2] = array_merge($v, $data[$k2]);
                    }
                }
            }
            foreach ($data as $key => $v) {
                $c = ($v['Rebate'] * ($v['Phone'] + $v['Message']));
                if ($c > 0) {
                    $data[$key]['Cost'] = round($v['Con_B'] / $c, 2);
                } else {
                    $data[$key]['Cost'] = "";
                }
            }
    ...
    //批量更新消费记录操作
    public function upd_promotion_con_all_do()
    {
        if (session('type') <= 2) {
            $input = input('data/a');
            $is_update = false;
            foreach ($input as $k => $v) {
                ...
                $input[$k]['Remarks'] = $v['ConRemarks']; //备注
                $input[$k]['Cli_Status'] = input('status'); //展示状态
            }
            if (!$is_update) {
                return ['code' => 1, 'msg' => '更新失败！更新条件不足！'];
            }
            $model = new ModelPromotionCon();
            $res = $model->allowField(true)->isUpdate()->saveAll($input);
            if ($res) {
                return ['code' => 0, 'msg' => '更新成功！'];
            } else {
                return ['code' => 1, 'msg' => '更新失败！请联系检查字段！'];
            }
        }
    }
    /*外部客户消费视图*/
